...
if [ "$5" != "default" ]; then
  if [ ! -f /opt/ide/bin/idea.vmoptions ] ; then
    echo "Cannot file /opt/ide/bin/idea.vmoptions options file"
    exit
  fi
  echo "-Didea.analyze.scope=$5" >> /opt/ide/bin/idea.vmoptions
  export STUDIO_VM_OPTIONS=/opt/ide/bin/idea.vmoptions
  export IDEA_VM_OPTIONS=/opt/ide/bin/idea.vmoptions

  if [ ! -f /opt/ide/bin/idea64.vmoptions ] ; then
    echo "Cannot file /opt/ide/bin/idea64.vmoptions options file"
    exit
  fi
  echo "-Didea.analyze.scope=$5" >> /opt/ide/bin/idea64.vmoptions
  export STUDIO_VM_OPTIONS=/opt/ide/bin/idea64.vmoptions
